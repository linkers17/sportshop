$(document).ready(function() {
   ...
   let menuLink = $('.menu__link-js'); //!!!--- реализовать функционал для кнопок 5 и 6

   menuLink.on('click', function(e) {     // Клик по кнопке меню

      e.preventDefault();

      let target = $(this).attr('data-target');
      let btnId = $(this).attr('data-id');
      let header = $('.header__box');

      $('.interface').load(target + '.php', function() {

         mask(btnId);

         if (btnId === '1') {

            $(`#suborder-${btnId}`).on('submit', function(e) {

               e.preventDefault();
   
               $.ajax({
                  url: `processorder-${btnId}.php`,
                  type: 'POST',
                  data: $(this).serialize(),
      ...
      fetch('./labs.json')
         ...
         .then((output) => {
            ...
            output.labs.forEach((item) => {

               if (item.target === target) {

                  header.append(`<span class="lab">${item.number}</span>`);
                  header.append(`<span class="theme">"${item.theme}"</span>`);
                  header.append(logoShop);
         ...
         .catch((reason) => {
            ...
            header.append(`<span class="message message_error">Что-то пошло не так:(</span>`);
   ...
   function mask(...ids) {
      ...
      $('.number-js').mask('9?99', {placeholder:" "});
   ...
   function register() {
      ...
      $('.login-js').mask('******?**************', {placeholder: " ", completed: verifyRegister()});
      ...
      $(`#register`).validate({
         ...
         messages: {
            ...
            email: {
               ...
               email: "Формат вашей почты должен быть: name@domain.com"
